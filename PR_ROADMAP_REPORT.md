...
| 9 | Fix visualizer to always draw annotations and add debug logging for rejections/suspects (autor: Copilot; criado 2026-01-08; merged 2026-01-08) | https://github.com/narutojgdr-sudo/Trekking-DragonBotZv2/pull/9 | merged | Yes | Cited & detailed | Módulo app.py + parâmetros debug.log_rejections/log_suspects | L140-150: logs de rejeições/suspects no loop | Manter; opcional: referenciar PR #9 em “Debug e visualização”. |
...
- **[Alta] Atualizar parâmetros críticos do tracker (PR #2)**
  ...
  - *Texto sugerido:* "Após a correção de deleção prematura (PR #2), os defaults recomendados são lost_timeout=3.0s, association_max_distance=250px, min_frames_for_confirm=4, ema_alpha=0.35 e grace_frames=20. Atualize o cone_config.yaml e valide em vídeo real." 

- **[Alta] Documentar source-aware logging e run logs (PR #18)**
  - *Título sugerido:* "Fonte de entrada (camera vs vídeo) e exportação JSONL"
  - *Texto sugerido:* "O app valida exclusividade entre camera.index e camera.video_path e pode exportar JSONL por frame com timestamps e source. Recomenda-se registrar as chaves debug.export_run_log, debug.run_log_dir e a anotação visual de source." 

- **[Média] Dashboard Streamlit (PRs #3, #4, #5, #6)**
  - *Título sugerido:* "Dashboard Streamlit para tuning remoto"
  - *Texto sugerido:* "Existe um dashboard multi-página em streamlit_app/ para editar cone_config.yaml via SFTP, com presets e validação. Documentar dependências (streamlit/paramiko), temas (dark mode) e fixes de SFTP/formatting." 
...
Se desejar, posso abrir issues automaticamente para PRs não documentadas (com template e checklist). Confirma?
